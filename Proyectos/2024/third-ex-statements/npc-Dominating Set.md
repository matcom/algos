...
Given a graph $$ G $$ and an integer $$ k $$, does there exist a subset of vertices of size at most $$ k $$ such that every vertex in the graph is either in the subset or adjacent to a vertex in the subset?

### Solution

Reduces from: Vertex Cover
Reduction Idea: The vertex cover can be transformed into a dominating set by including vertices that cover all edges, ensuring that every vertex is either included or adjacent to a covered vertex.
